...
// === AUDIO POSISIONAL (nempel ke planet) ===
const galaxySound = new THREE.PositionalAudio(listener);
...
function fadeInAudio() {
  if (!galaxySound || !galaxySound.isPlaying) return;

  audioFade = Math.min(audioFade + 0.01, targetVolume);
  galaxySound.setVolume(audioFade);

  if (audioFade < targetVolume) {
    requestAnimationFrame(fadeInAudio);
  }
}

audioLoader.load('audio/LANY-you.mp3', (buffer) => {
  galaxySound.setBuffer(buffer);
  galaxySound.setLoop(true);
  // jarak audio
  galaxySound.setRefDistance(20);     // jarak normal
  galaxySound.setMaxDistance(200);    // mulai hilang
  galaxySound.setRolloffFactor(1.5);  // efek fade jarak
});

// tempelkan ke planet
planet.add(galaxySound);

//Ganti Deskripsi Planet
const ringTexts = [
  'Galaxy of love From Sinzz',//untuk deskripsi planet layer 1
  "I love you",//untuk deskripsi planet layer 2
  "♡ Happy Valentine ♡",//untuk deskripsi planet layer 3
  "14/02/2026",//untuk deskripsi planet layer 4
...
function onCanvasClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (!galaxySound.isPlaying) {
      audioFade = 0;              // reset volume
      galaxySound.setVolume(0);   // mulai dari diam
      galaxySound.play();
      fadeInAudio();              // mulai fade in
    }
